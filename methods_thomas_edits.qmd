...
The model has 91 residuals less than or equal to 0, and 9 residuals that are greater than 0. 

```{r}
residuals <- resid(model)
pos_residuals <- residuals[which(residuals > 0)]
summed_pos_residuals <- sum(pos_residuals)
```
