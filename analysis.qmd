...
This data fails to meet the assumptions of OLS, requiring the usage of quantile regression, if one seeks to interpret the betas of regression.

### Assumptions of quantile regression vs OLS

The main difference in the assumptions between quantile regression and OLS is that OLS assumes homoskedasticity and normally distributed errors. Quantile regression only assumes that the quantiles of the conditional distribution are linear and well defined.
...
In the figure, the house sale prices are plotted against their gross living areas, and an OLS regression line is fit to the data. Evidently, the data exhibits heteroskedasticity, as the observations are spread out at greater ranges from the regression line as gross living area increases, thus breaking the assumption of homoskedasticity of OLS, but this is perfectly acceptable for quantile regression.

In order to show that the errors are not normally distributed, the figure below is a plot of 5 distributions. The distributions are made of the data from the above scatter plot, except the gross living area has been split into 5 bins. The distributions are the distribution of sale prices for each bin.

```{r}
p2
```

The heteroskedasticity of the data can be seen above as well as the previous chart in that each successive distribution, as gross living area increases, has a greater range and more outliers. The last 2 distributions can be ignored as, when referring back to the scatterplot it is apparent that there is not enough data for these to mean anything--- the scatterplot alpha has been reduced to further illustrate the sheer density of points in the cluster on the left. Looking at the first 3 it is clear the distributions progressively spread out. Furthermore, while the first distribution is approximately normal, the 2nd and 3rd are skewed towards the more expensive homes, with many expensive outliers and 0 outliers towards the affordable side. These skewed distributions show that the **conditional distribution** of sale price is not normal at multiple levels of the predictors.

The above chart is merely an empirical illustration that attempts to show what the conditional distribution might look like at various cross-sections however it is of course not the true theoretical/underlying conditional distribution. In any case, the illustration is enough to support the fact that the errors of regression will not be normally distributed. Furthermore, the boxplots overlapping the distributions are enough to support the assumption of quantile regression. Quantile regression assumes the quantiles are clearly defined and linear. We can see this is the case by noting the 75% quantile--- the upper bar of each boxplot--- of the first 3 boxplots could easily be connected by a line. This is also the case for the median and the 25% quantile. This makes the data at hand a perfect dataset for leveraging the power of quantile regression. Meeting the assumptions of a model means that the resulting regressions will be highly efficient and generate an implied conditional distribution that can be used to make predictions for yet unseen observations.
...
taus <- seq(from=0.05, to=.95, by=.01)
qrs = rq(data=df, SalePrice ~ GrLivArea + LotArea + TotRmsAbvGrd + as.factor(LotShape) + as.factor(Foundation), tau=taus)
qrs_grLivArea = rq(data=df, SalePrice ~ GrLivArea + LotArea + TotRmsAbvGrd + as.factor(LotShape) + as.factor(Foundation), tau=taus)
...
```{r, warning=FALSE}
ols_grlivarea = lm(data=df, SalePrice ~ GrLivArea)
print(ols_grlivarea$coefficients)
num_of_rows <- nrow(qrs$coefficients)
#par(mfrow = c(2, 2))
#plot.new()
#mtext("Beta change per quantile level", side = 3, line = - 2, outer = TRUE)
#plot(ols)
plot(summary(qrs, se="iid"), xlab="Quantiles", ylab="Y-Intercepts", type="l", mfrow = c(2,2))
#abline(h=ols_grlivarea$coefficients[2], col="blue", type="l")
#plot(qrs$coefficients[2, ], xlab="Quantiles", ylab="Lot-Area", type="l")
#plot(qrs$coefficients[3, ], xlab="Quantiles", ylab="TotRmsAbvGrd", type="l")
#plot(qrs$coefficients[4, ], xlab="Quantiles", ylab="LotShape-IR2", type="l")
mtext("Beta change per quantile level", side = 3, line = - 2, outer = TRUE)
#plot(qrs$coefficients[5, ], xlab="Quantiles", ylab="LotShape-IR3", type="l")
#plot(qrs$coefficients[6, ], xlab="Quantiles", ylab="LotShape-Reg", type="l")
#plot(qrs$coefficients[7, ], xlab="Quantiles", ylab="Foundation-CBlock", type="l")
#plot(qrs$coefficients[8, ], xlab="Quantiles", ylab="Foundation-PConc", type="l")
mtext("Beta change per quantile level", side = 3, line = - 2, outer = TRUE)
#plot(qrs$coefficients[9, ], xlab="Quantiles", ylab="Foundation-Slab", type="l")
#plot(qrs$coefficients[10, ], xlab="Quantiles", ylab="Foundation-Stone", type="l")
#plot(qrs$coefficients[11, ], xlab="Quantiles", ylab="Foundation-Wood", type="l")
...
In the first table for predictions for the whole data, we can see that OLS and quantile regression have similar error metrics, with OLS having better min max values but that is because it cares a lot more to outliers therefore making these metrics better.

However, if we look at the second table for results for predicting the top and bottom 10% of the data, we can see that the specialized quantile regression has better min max error values and are more accurate and consistent across the board. The QR with tau = 0.1 is better than OLS when predicting the bottom 10% priced houses, and the QR with tau = 0.9 is better than OLS when predicting the top 10% priced houses. The top 10% priced houses comparison minimum error is won by OLS but that could be due to a very far outlier and OLS caring about it more, or that it is affected by a variable that we're not evaluating in this study.
...
As demonstrated in the previous 2 tables when illustrating both the categorical and the continuous variables, the p-value of each variable changes with the changing of the quantile regressions' quantile. It can even switch from significant to non-significant. That is the case with lot shape in the 0.95th quantile. It was significant in all previous quantiles except 4in the 95th quantile. The same goes for the number of the total rooms Gross living. It was significant across all quantiles but is insignificant for the 90th quantile. On the other hand, OLS assumes that the significance is constant throughout the whole data set, and is unable to capture or understand this variance.

## Summary

To put our results into real world perspective, we have to take a step back and look at the bigger picture. In our example people normally when buying a house won't be looking across all the house price range, but they would probably only focus on a specific range of house prices, whether it be expensive, cheap, or moderate. So, this is when quantile regression shines as it is a lot better than regular mean dependent models like OLS in predicting a specialized/focused dataset that doesn't represent the whole population distribution. One more thing is that as demonstrated in our study, real world datasets are not normalized, and are not free from outliers, therefore when using OLS, it will suffer a lot from handling these types of data while quantile regression should be able to handle it in a much better way, providing more accurate results. Finally, in order to make statistical conclusions and gain the ability to interpret the betas of regression, model assumptions must be met. The data used in this paper is very much like rear world data, skewed conditional distribution and non-constant variance of errors. OLS betas are highly inefficient in this type of scenario, for the highest level of statistical accuracy, a more flexible model like quantile regression must be used.
