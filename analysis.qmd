...
```{r, 'Getting Data housing', echo=FALSE}
...
QR for 50th quantile has a lower MAE therefore it is has more accurate predictions but they are almost the same.

```{r 'MAE for different quantiles'}
...
ols_q90_mae <- mae(predict(ols, newdata=Q90SalePrice), Q90SalePrice$SalePrice)
ols_q90_mae
q90_mae <- mae(predict(qr90_p, newdata=Q90SalePrice), Q90SalePrice$SalePrice)
q90_mae
...
ols_q10_mae <- mae(predict(ols, newdata=Q10SalePrice), Q10SalePrice$SalePrice)
ols_q10_mae
q10_mae <- mae(predict(qr10_p, newdata=Q10SalePrice), Q10SalePrice$SalePrice)
q10_mae

```

When comparing Quantile regression for tau = 0.90 with OLS while predicting the top 10% of data when sorted by sales amount. The OLS model had MAE value of `r format(round(ols_q90_mae, digits=2), scientific=F)`, while QR had MAE value of `r format(round(q90_mae, digits=2), scientific=F)`.
And when comparing Quantile regression for tau = 0.10 with OLS while predicting the bottom 10% of data when sorted by sales amount. The OLS model had MAE value of `r format(round(ols_q10_mae, digits=2), scientific=F)`, while QR had MAE value of `r format(round(q10_mae, digits=2), scientific=F)`.
So QR has better MAE than OLS when predicting data that is far off the mean. That is a big advantage of QR as it has the flexibility to provide models for a specific set of the data.
...
Since OLS' goal is to minimize RMSE, as expected it has a better (lower) value. But QR has a very similar value which shows how well QR model can keep up even if it is not focusing on optimizing RMSE.

```{r 'Root mean squared error for specific quantiles'}
ols_q90_rmse <- rmse(predict(ols, newdata=Q90SalePrice), Q90SalePrice$SalePrice)
ols_q90_rmse
q90_rmse <- rmse(predict(qr90_p, newdata=Q90SalePrice), Q90SalePrice$SalePrice)
q90_rmse

ols_q10_rmse <- rmse(predict(ols, newdata=Q10SalePrice), Q10SalePrice$SalePrice)
ols_q10_rmse
q10_rmse <- rmse(predict(qr10_p, newdata=Q10SalePrice), Q10SalePrice$SalePrice)
q10_rmse
```

When comparing Quantile regression for tau = 0.90 with OLS while predicting the top 10% of data when sorted by sales amount. The OLS model had RMSE value of `r format(round(ols_q90_rmse, digits=2), scientific=F)`, while QR had RMSE value of `r format(round(q90_rmse, digits=2), scientific=F)`.
And when comparing Quantile regression for tau = 0.10 with OLS while predicting the bottom 10% of data when sorted by sales amount. The OLS model had RMSE value of `r format(round(ols_q10_rmse, digits=2), scientific=F)`, while QR had RMSE value of `r format(round(q10_rmse, digits=2), scientific=F)`.
So QR also has better RMSE than OLS when predicting data that is far off the mean, even though OLS is supposed to minimize RMSE value.
...
#tau: 0.1
qrs_summary[11][[1]]$coefficients[,4]
#tau: 0.25
qrs_summary[26][[1]]$coefficients[,4]
#tau: 0.5
qrs_summary[51][[1]]$coefficients[,4]
#tau: 0.75
qrs_summary[76][[1]]$coefficients[,4]
#tau: 0.9
qrs_summary[91][[1]]$coefficients[,4]
```
